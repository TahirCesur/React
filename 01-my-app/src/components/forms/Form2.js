...
const Form2 = () => {
  ...
  const [mesaj, setMesaj] = useState(""); //! Alert için
  const [loading, setLoading] = useState(false); //? Spinner için
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true); //? Spinner için
    setMesaj(""); //! Alert için
    // Burası fake API
    setTimeout(() => {
      setMesaj("Bilgileriniz başarılı bir şekilde gönderildi"); //! Alert için
      setLoading(false); //? Spinner için
    }, 3000);
  };
